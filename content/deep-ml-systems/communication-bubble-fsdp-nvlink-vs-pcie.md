...
In the PCIe scenario, approximately **163 ms** of NCCL activity was hidden behind compute (192.61 ms total NCCL minus the 28.91 ms that materialized as real stall). The 28.90 ms "communication bubble" is precisely the portion of network latency that was too slow to hide — the amount by which communication time *exceeded* the computation window and forced the GPU to wait.
